---
title: "Regression modelling - Simple linear regression"
...
- $\beta_1$ is the population *slope* parameter, and like $\alpha$, is also **fixed**.
- $\epsilon_i$ is the error associated with predictions of $y_i$, and unlike $\alpha$ or $\beta$, it is *not fixed*.
...
What is the predicted child heights for parents of height 70, 72, and 73 inches?

```{r}
predict(fit, data.frame(parent = c(70, 72, 73)))
```
